...
Object.keys(categories).forEach(catTag => categoriesList.push(categories[catTag]));


categoriesList.forEach(category => {
  if (usedImages.indexOf(category.image) !== -1) {
    const newImageProduct = category.products.find(p => usedImages.indexOf(p.image) === -1);
    if (newImageProduct) {
